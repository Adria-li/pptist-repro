...
export default () => {
  ...
  function shapeToSVGElement(el: PPTShapeElement): SVGSVGElement {
    const fill = el.fill || 'black'
    const stroke = el.outline?.color || ''
    const strokeWidth = el.outline?.width || 0
    const viewBox = el.viewBox || [el.width, el.height]
    const width = el.width || viewBox[0]
    const height = el.height || viewBox[1]
    ...
    const svgString = `
      <svg overflow="visible" width="${width}" height="${height}">
        ...
        <g transform="scale(${scaleX}, ${scaleY}) translate(0,0) matrix(1,0,0,1,0,0)">
          <path vector-effect="non-scaling-stroke" stroke-linecap="butt" stroke-miterlimit="8"
            d="${el.path}" fill="${fill}" stroke="${stroke}" stroke-width="${strokeWidth}" stroke-dasharray="0 0"></path>
        </g>
      </svg>
    `.trim()
  ...
  const exportJSON = () => {
    ...
    for (const slide of slidesForExport) {
    ...
    for (const slide of _slides.slides) {
      ...
      for (const el of slide.elements) {
        if (el.type === 'text') {
          ...
          if (el.outline && el.outline.width && el.outline.color) {
            options.border = {
